...
namespace PortfolioUnleashed.Data_Layer
{
    public class DatabaseDAL : IDAL
    {
        ...
        public List<Project> retrieveAllProjectsByUserID(int userID)
        {
            using (var db = new PortfolioUnleashedContext())
            {
                return db.Projects.Include("Contributions").Include(p => p.Contributions.Select(c => c.ContributionMedia)).Include(p => p.Contributions.Select(c => c.User)).Include("ProjectMedia").Include("Portfolios").Include("ProjectPermissions").Where(p => p.Contributions.Any(c => c.UserId == userID)).ToList();
            }
        }
