...
namespace PortfolioUnleashed.Controllers
{
    public class SearchController : Controller
    {
        ...
        public ActionResult Portfolios(string searchText, string searchType, string orderBy, bool isDescending = false)
        {
            if (searchText == null || searchType == null)
            {
                return View(viewName: "Portfolios");
            }
            else if (searchType.Count() >= 9 && searchType.Substring(0, 9) == "portfolio")
            {
                ...
                {
                    portfolioResults.Add(new VMPortfolioSearchResult { OwnerId = portfolio.UserId, OwnersFirstName = portfolio.User.FirstName, OwnersLastName = portfolio.User.LastName, PortfolioDescription = portfolio.Description, PortfolioId = portfolio.Id, PortfolioTitle = portfolio.Title});
                }
